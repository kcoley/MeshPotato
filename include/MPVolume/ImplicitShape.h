...
namespace MeshPotato {
	namespace MPVolume {
		class ImplicitSphere : public MeshPotato::MPVolume::Volume<float> {
			public:
				static boost::shared_ptr<ImplicitSphere> create(const float &_R,  const MeshPotato::MPUtils::MPVec3 &_C) {
		...
		class Clamp: public MeshPotato::MPVolume::Volume<T> {
			...
			static boost::shared_ptr<Volume<T> > create(const boost::shared_ptr<MeshPotato::MPVolume::Volume<T> >&_f,  const float _e0, const float _e1) { 
